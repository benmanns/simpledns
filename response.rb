...
class Dns::Response
  ...
  class << self
    def parse data
      response = new
      response.id, response.flags, questions_length, answers_length, authorities_length, additionals_length = data.unpack('nnnnnn')
      index = 6 * 2
      questions_length.times do
        question, index = Dns::Question.parse(data, index)
        response.questions << question
      end
      answers_length.times do
        answer, index = Dns::Answer.parse(data, index)
